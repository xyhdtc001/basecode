...
ref_ptr<MemStream>
VHttp::getHttpStream( const Data& file, uint32 retryNum, int Timerout )
{
	int32 pos;
	if ( (pos=file.find("/")) == -1 )
		return NULL;
	...
	return getHttpStream( server, httpfile, -1, retryNum, Timerout );
}


ref_ptr<MemStream>
VHttp::getHttpStream( const Data& server, const Data& httpfile, int32 len, uint32 retryNum, int Timerout )
{
	...
	for ( uint32 retryIndex=0; retryIndex<retryNum; retryIndex++ )
	{
		...
		if ( !_clientSock.connect(Timerout) )
		{
