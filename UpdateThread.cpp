	...
	*/
	...
	for ( std::vector<FileItem>::iterator iter=_remoteVersion.vecFile.begin(); iter!=_remoteVersion.vecFile.end(); iter++ )
	{
		...
		else
		{
			...
			if (0 < vecUpdateItem.size() && iter->file.getExtName() == "xml")
			{
				vecUpdateItem.push_back( *iter );
			}
		}
	...
	{
		SendMessage( _hWnd, WM_UPDATE_EVENT, UPDATE_UPDATE, 0 );
	}
	...
	if ( vecUpdateItem.size() > 0 )
	{
		...
		for ( std::vector<FileItem>::iterator iter=_localVersion.vecFile.begin(); iter!=_localVersion.vecFile.end(); iter++ )
		{
			...
			std::string ver = Data::ToString(iter->ver).c_str();
			std::string name = iter->name.c_str();
			std::string file = iter->file.c_str();
			std::string path = iter->path.c_str();
			fileNode->append_attribute( doc.allocate_attribute("ver", doc.allocate_string(ver.c_str())) );
			fileNode->append_attribute( doc.allocate_attribute("name",doc.allocate_string(name.c_str())) );
			fileNode->append_attribute( doc.allocate_attribute("file",doc.allocate_string(file.c_str())) );
			if ( !iter->path.isEmpty() )
				fileNode->append_attribute( doc.allocate_attribute("path",doc.allocate_string(path.c_str())) );
