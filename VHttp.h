...
public:
	...
	ref_ptr<MemStream> getHttpStream( const Data& server, const Data& httpfile, int32 len=-1, uint32 retryNum=1, int Timerout=-1 );
	...
	ref_ptr<MemStream> getHttpStream( const Data& file, uint32 retryNum=1, int Timerout=-1 );
